...
const USER_SHEET_ID = "17lpaLBAL9XidYqiMhKNWRhZdEIqa0OzuVP7SYYc6VfQ";

function doGet(e) {
  e = e || {};
  if (e.pathInfo == "events") return getEvents_(e);
  if (e.pathInfo == "book") return bookSlot_(e);
  if (e.pathInfo == "me") return getCurrentUser_(e);
...
// ===== Review Meeting Calendar Functions =====
function getCurrentUser_(e) {
  const email = Session.getActiveUser().getEmail();
  const user = checkAuth_(email);
  return json_(user);
}
...
function getEvents_(e) {
  ...
  const now = new Date();
  const timeMin = toIso_(now);
  const timeMax = toIso_(new Date(now.getTime() + 30 * 24 * 3600 * 1000));
  const resp = Calendar.Events.list(calId, {
    timeMin: timeMin,
    timeMax: timeMax,
    singleEvents: true
  });
  const items = (resp.items || []).map(ev => {
    let owner = false;
    try {
      const desc = JSON.parse(ev.description || '{}');
      owner = desc.uid === user.id;
    } catch (err) {}
    return {
      id: ev.id,
      title: ev.summary,
...
function bookSlot_(e) {
  ...
  const created = Calendar.Events.insert(ev, calId);
  return json_({
    status: 201,
    id: created.id,
...
function checkAuth_(email) {
  ...
  if (!sh) throw new Error('401');
  const data = sh.getDataRange().getValues();
  const hdr = data[0];
  const idxEmail = hdr.indexOf('email');
  const idxId = hdr.indexOf('id');
  const idxRole = hdr.indexOf('role');
  const idxLang = hdr.indexOf('lang');
  const idxName = hdr.indexOf('name');
